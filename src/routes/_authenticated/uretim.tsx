...
function KanbanScreen() {
  ...
  const [station, setStation] = useState(search.istasyon ?? ALL);
  const [priority, setPriority] = useState(search.oncelik ?? ALL);
  const [machine, setMachine] = useState(ALL);
  const [operator, setOperator] = useState(ALL);
  const [late, setLate] = useState(search.geciken ?? false);
  const [blockedOnly, setBlockedOnly] = useState(search.bloke ?? false);
  const [reworkOnly, setReworkOnly] = useState(search.rework ?? false);
  const [criticalOnly, setCriticalOnly] = useState(search.kritik ?? false);
